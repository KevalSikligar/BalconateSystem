<%@ Page Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<IndexModel<Article>>" %>
...
<div class="wide-main">
    ...
    <div id="articles-menu" class="display-none">
        <%using (Ajax.BeginForm("GetArticles", null, null, new AjaxOptions { HttpMethod = "POST", OnSuccess = "onsuccess", OnComplete = "oncomplete" }, new { id = "frmLocate" })) { %>	
    ...
    <div id="results">
        ...
        <% var url = Url.Action("articles", article.Area.ID.ToString(), new { id = article.Url }); %>
        ...
        <% } %>
        <% } else { %>
        <span class="noitems"><%:IndexModel<Article>.NO_ITEMS_TEXT %></span>
        <% } %>
    </div>
</div>
</asp:Content>
...
<script type="text/javascript">
    ...
    function onsuccess(data, textStatus, xhr) {
        ...
        if ($("#results div.article").length == 0) {
            $("<span />")
                .addClass("noitems")
                .text("<%=IndexModel<Article>.NO_ITEMS_TEXT %>")
                .appendTo($("#results"));
        }
    }
</script>
