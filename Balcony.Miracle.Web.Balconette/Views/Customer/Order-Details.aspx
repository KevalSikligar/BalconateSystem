...
<div class="main">
	<ul id="cookie-menu">
		...
		<li class="last">Order Details</li>
	</ul>
    <h1>Billing and Delivery Details</h1>
    <div id="details_wrap">
        ...
        <div>
            ...
            <% using(Ajax.BeginForm("sign-in", null, new AjaxOptions{ HttpMethod = "POST"}, new { id="frm_login" } )) {%>
                <input type="hidden" name="Redirect" value="<%=Url.Action("order-details") %>"/>
                <div>
                    <p>
                        Please fill in your email and password and click 'Sign In' button.
                    </p>
                    <%:Html.Partial("SignIn", new SignInModel()) %>
                    <div class="validation-summary-errors"></div>
                    <p class="paddingl115">
                        <input type="submit" class="area_color k-button btnProceed prog1" value="Sign In ..." />
                    </p>
                </div>
                <div>
                    <p>
        ...
        <div>
            ...
            <% using(Ajax.BeginForm(null, null, new AjaxOptions{ HttpMethod = "POST"}, new { id="frm_signup" } )) {%>
	            <p>
	                Please fill in sections 1 - <%:numofsections %>. Then click the 'Proceed to Payment' button.
	            </p>
            ...
            <input type="hidden" name="subregID" id="subregID" value="<%:Html.DisplayFor(m=>m.SubRegionId) %>" />
                <h3 class="delivery-options-title">2. Delivery options</h3>
                ...
                <% if(!Context.IsCustomer()){ %>
                <h3>3. Password (for future reference)</h3>
                <p>
                ...
                <div class="validation-summary-errors"></div>
                <p class="paddingl115">
                    <input type="submit" class="area_color k-button btnProceed prog1" value="Proceed to Payment ..." />
                </p>
            <% } %>
        </div>
    </div>
