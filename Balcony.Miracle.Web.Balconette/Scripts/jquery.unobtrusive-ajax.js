...
(function ($) {
    ...
    function asyncOnSuccess(element, data, contentType) {
        ...
        if (contentType.indexOf("application/json") !== -1) {
            if (data.__kind__ == 11) { //validation errors
                var vld = $(element).validate();
                if (vld) {
                    vld.showErrors(data.errors);
                }
                if (data.general) {
                    var div = $(element).find(".validation-summary-errors");
                    if (div.length > 0) {
                        var ul = $("<ul/>");
    ...
    function asyncRequest(element, options) {
        ...
        $(element).find(".validation-summary-errors").empty();
