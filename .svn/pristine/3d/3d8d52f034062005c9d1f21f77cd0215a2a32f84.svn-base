...
<asp:Content ContentPlaceHolderID="BodyContent" runat="server">
    <h1>Downloads Index</h1>
    <div id="boxes">
        <%using (Ajax.BeginForm("GetDownloads", "Downloads", null, new AjaxOptions { HttpMethod = "POST", OnSuccess = "onsuccess", OnComplete = "oncomplete" }, new { id = "frmLocate" }))
          { %>	
...
<asp:Content ID="Content2" ContentPlaceHolderID="FooterEnd" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            $('#save').click(function () {
                $("#prog1").show();
                saveEditableFiles("<%=Url.Action("bulkedit")%>", function () {
                    $("#prog1").hide();
                    alert("save ok");
                });
            });
        ...
        function onsuccess(data, textStatus, xhr) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var div = $("#" + id);
                if (div.length == 0) {
                    div = createEditableFile("<%=Url.Action("gettags", "tags", new { area = "" })%>", image)
                        .attr("id", id)
                        .appendTo($("#results"));
                }
            }
